...
namespace OctopusController
{
    ...
    public class MyScorpionController
    {
        ...
        public void InitLegs(Transform[] LegRoots, Transform[] LegFutureBases, Transform[] LegTargets)
        {
            ...
            yLeg = new float[_legs.Length];
            aux = new Vector3[_legs[0].Bones.Length];
            distances = new float[_legs[0].Bones.Length];
            baseSaved = new Vector3[_legs.Length];
            posSaved = new Vector3[_legs.Length];

        }

        public void InitTail(Transform TailBase)
        {
            scorpionScale = 0.3262f;
        ...
        private void updateLegPos()
        {
            ...
            for (int i = 0; i < _legs.Length; i++)
            {
                ...
                if (Vector3.Distance(_legs[i].Bones[0].position, legFutureBases[i].position) > 0.5f) //0.35
                {
                    baseSaved[i] = legFutureBases[i].position;
                    posSaved[i] = _legs[i].Bones[0].position;
                    Vector3 raycasPos = baseSaved[i] + new Vector3(0, 5, 0);
                ...
                if (isLerp[i] == true)
                {
                    ...
                    Vector3 pos = new Vector3 (baseSaved[i].x, yLeg[i], baseSaved[i].z);
                    _legs[i].Bones[0].position = Vector3.Lerp(posSaved[i], pos, lerpCounter[i]);
                    lerpCounter[i] += 0.1f;
                }
        ...
        float DistanceFromTarget(Vector3 target, float[] angles)
        {
            ...
            Vector3 endEffector = _tail.Bones[0].position;
            for (int i = 1; i < _tail.Bones.Length; i++)
            {
                rot *= Quaternion.AngleAxis(angles[i - 1], tailBonesAxis[i - 1]);
                endEffector += rot * tailBonesStartOffset[i] * scorpionScale; 
            }
            Vector3 p = endEffector;
            return Vector3.Distance(target, p);
        }
